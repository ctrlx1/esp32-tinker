...
tinker::ProjectDefinition StarterProject::definition() const {
  return {"starter-hub75",
          "Starter HUB75",
          APP_VERSION,
          "Starter-H75-",
          {"starter_hub75", "cfgVer", kSettingsVersion}};
}

bool StarterProject::migrateSettings(Preferences &,
...
void StarterProject::loadSettings(Preferences &preferences) {
  ...
  Serial.print(" starter-hub75 brightness=");
...
bool StarterProject::applyPortalRequest(
...
void StarterProject::drawHelloWorld() {
  if (!runtime_.hasText() || !runtime_.hasColor()) {
    Serial.println("Starter HUB75 display is missing text or color.");
    started_ = false;
    return;
  }
...
void StarterProject::startPrograms() {
  hue_ = 0;
  lastFrameMs_ = 0;
  drawHelloWorld();
}
